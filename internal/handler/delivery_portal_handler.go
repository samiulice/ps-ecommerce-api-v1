...
// GetPortalProfile handles GET /delivery/portal/profile
func (h *DeliveryHandler) GetPortalProfile(w http.ResponseWriter, r *http.Request) {
	employeeID, ok := middleware.AuthIDFromContext(r.Context())
	if !ok || !middleware.IsEmployee(r.Context()) {
		utils.Unauthorized(w, errors.New("unauthorized delivery user"))
		return
	...
	profile, err := h.svc.GetPortalProfile(r.Context(), int64(employeeID))
	if err != nil {
		utils.ServerError(w, err)
		return
	}
