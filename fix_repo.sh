#!/bin/bash
# Remove the bad functions at the bottom of delivery_repository.go (lines added by my last cat)

cat internal/repository/delivery_repository.go | sed '/func (r \*DBRepository) ListDeliveryMen/,$d' > internal/repository/delivery_repository.go.tmp
mv internal/repository/delivery_repository.go.tmp internal/repository/delivery_repository.go

cat << 'INNER_EOF' >> internal/repository/delivery_repository.go

func (r *DBRepository) ListDeliveryMen(ctx context.Context) ([]model.DeliveryMan, error) {
        query := `
                SELECT 
...
func (r *DBRepository) ListDeliveryMethods(ctx context.Context) ([]model.DeliveryMethod, error) {
...
type OrderDeliveryHistory struct {
        model.OrderDelivery
        DeliveryManName  sql.NullString \`json:"delivery_man_name,omitempty"\`
        DeliveryManPhone sql.NullString \`json:"delivery_man_phone,omitempty"\`
}

func (r *DBRepository) GetDeliveryHistory(ctx context.Context, limit, offset int) ([]OrderDeliveryHistory, error) {
        query := `
                SELECT 
